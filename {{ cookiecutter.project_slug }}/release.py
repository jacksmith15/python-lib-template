...
def repo_compare(
    ...
    return (
        f"[{new or 'Unreleased'}]: http://github.com/{{ cookiecutter.github_user }}/"
        f"{{ cookiecutter.project_slug }}/compare/{old or 'initial'}..{new or 'HEAD'}\n"
    )
...
def update_versions(current_version: Version, new_version: Version):
    with open(package.__file__, "r", encoding="utf8") as file:
        ...
        new_init = [
            *takewhile(
                lambda l: not re.match(r"^__version__ = .*", l),
                file
            ),
