...
typecheck() {
    ...
    mypy --ignore-missing-imports {{ cookiecutter.package_name }} tests || EXIT_CODE=1
}

tests() {
    header "Running unit tests" "$blue"
    pytest -v -s --junitxml=unit_test_results.xml --cov="{{ cookiecutter.package_name }}" --cov-append --cov-branch --cov-report= tests || EXIT_CODE=1
}
